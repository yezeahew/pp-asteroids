...
class Observable<Input> {
  ...
  /**
   * The observable notifies just once only with the specified delay
   * @param milliseconds Delay time of observable notifications
   * @return Observable payload is total elapsed time
   * 
   * This is used to implement the bullet_range feature. 
   * The bullet will be 'killed' after a certain time. This makes the game more realistic.
   */
  static timeout<V>(milliseconds: number): Observable<V> {
  ...
  /**
   * creates a child Observable that fires only the first count times
   * @param count thu numbers to fire
   * @return child Observable that fires the first count times
   * 
   * This was initially used to implement bullet features. However, that implementation was
   * removed after some modification.
   */
  takeN(count: number): Observable<Input> {
    if (count > 0) {
      return new Observable<Input>(observer=>{
        const unsub = this.subscribe(e=>{
          observer.next(e)
          unsub()
          this.takeN(count - 1).subscribe(e=>observer.next(e), ()=>observer.complete())
        }, ()=>observer.complete())
        return unsub
      })
    }
  ...
  /**
   * creates a child Observable that fires with milliseconds cool down time
   * @param milliseconds the cool down time
   * @return child Observable with cool down time
   * 
   * This is used to implement the bullet cool down feature.
   */
  timer(milliseconds: number): Observable<Input> {
    return new Observable<Input>(observer=>{
      return this
        .takeN(1)
        .concat(Observable.timeout(milliseconds))
        .subscribe(e=>observer.next(e), ()=>{
          this.timer(milliseconds)
            .subscribe(e=>observer.next(e), ()=>observer.complete())
        })
    })
  }

  /**
   * Concat two Observables which switches to the second one when the first one completes
   * @param o Observable to be concatenated
   * @return child Observable of the concatenation
   * 
   * This was initially used, however, the code was removed after modifications.
   */
  concat(o: Observable<Input>): Observable<Input> {
    return new Observable<Input>(observer => {
      return this.subscribe(e=>observer.next(e), ()=>{
        o.subscribe(e=>observer.next(e), ()=>observer.complete())
      })
    })
  }
